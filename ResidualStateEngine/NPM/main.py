...
class ResidualPolicyEnsembleQC(QCAlgorithm):
    def initialize(self):
        ...
        self.rebalance_days = 7  # rebalanceo semanal para reducir costos
        self.selection_days = 21  # seleccion cada 3 semanas
        self.target_leverage = 2.5  # leverage aumentado
        ...
        self.sigma = 0.15  # mayor exploracion
        self.beta = 0.35  # balance diversidad/score

        self.turnover_penalty = 0.25  # mayor penalizacion turnover



        # Trading mas conservador
        self.trade_threshold = 0.15  # menos sensible para reducir turnover
    ...
    def _rebalance(self):
        ...
        # Detectar regimen de mercado para ajustar leverage
        rets = np.array(self.returns, dtype=float)
        recent_vol = rets[-60:].std() if len(rets) >= 60 else rets.std()
        vol_percentile = min(1.0, recent_vol / 0.015)  # normalizar a vol tipica
        
        # Aumentar leverage en baja vol, reducir en alta vol
        leverage_mult = 1.0 + (1.0 - vol_percentile) * 0.8
        
        # Trend filter: solo largo si tendencia positiva
        prices = np.array(self.prices, dtype=float)
        ema50 = self._ema(prices, 50)
        ema200 = self._ema(prices, 200)
        in_uptrend = prices[-1] > ema50 and ema50 > ema200
        
        # Aplicar leverage dinamico
        if in_uptrend:
            p_total = float(np.clip(p_total, -1.0, 1.0)) * self.target_leverage * leverage_mult
        else:
            # Reducir exposicion en downtrend
            p_total = float(np.clip(p_total, -1.0, 1.0)) * self.target_leverage * 0.5
        
        # Forzar largo en fuerte uptrend
        if in_uptrend and p_total < 0.5:
            p_total = max(p_total, 0.8)
